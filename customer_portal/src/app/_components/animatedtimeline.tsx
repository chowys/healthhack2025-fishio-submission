...
interface TimelineItemProps {
  ...
  onEventClick: (event: TimelineEvent) => void; // Updated to pass the entire event
}
...
const TimelineItem: React.FC<TimelineItemProps> = ({
    ...
    styles,
    customRender,
    onEventClick, // Add a prop to pass the event ID back
  }) => {
    ...
    return (
      ...
      >
        ...
        {/* Timeline Content */}
        <motion.div
          className={cn("flex-grow leading-5 cursor-pointer rounded-lg p-4", !isLast && "mb-10")}
          onClick={() => {
            onEventClick(event);
            alert(`Event: ${event.title}\nDescription: ${event.description}\nDate: ${event.date}`);
          }}
          whileHover={{ backgroundColor: "#d4f5d4" }}
        >
          <>
...
export function AnimatedTimeline({
  ...
  const handleHover = (index: number | null) => {
    ...
    // Logs when onEventClick is called, ensure it's passed properly
    const handleEventClick = (event: TimelineEvent) => {
        console.log("Event clicked in parent:", event);
        onEventClick?.(event); // Make sure the parent's onEventClick is called
      };

  return (
    <div className={`relative py-4 ${className}`}>
      {events.map((event, index) => (
        <div key={event.id} onClick={() => { console.log(event.id);}}>
          <TimelineItem
              event={event}
