...
import AnimatedTimelinePage from "../_components/animatedtimeline";
...
const AppointmentsPage = () => {
    ...
    const userId = "1ZRIipkHL1Nx3ZiQuk6W9kd2zx43"; // Replace with dynamic user ID if needed
    ...
    const handleNavigation = (appointment_id: string) => {
        router.push(`/exercisePlan/${appointment_id}`);
    };
    ...
    return (
        <div className="flex h-screen">
            {/* Sidebar */}
            <aside className="w-full md:w-1/4 min-h-screen p-6 border-r">
                <AnimatedTimelinePage
                    containerClassName=""
                    customEventRender={(event) => (
                        <div onClick={() => {
                            const currentWeeklyPlanId = (event as TimelineEvent).weeklyPlan_id;
                            console.log(">>>>!", currentWeeklyPlanId);
                            setSelectedWeeklyPlanId(currentWeeklyPlanId);
                            console.log(currentWeeklyPlanId);
                            console.log("selectedWeeklyPlanId1", selectedWeeklyPlanId);
